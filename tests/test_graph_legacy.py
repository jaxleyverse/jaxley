...
"""Tests for the legacy SWC reader and graph pipeline (`jaxley.io.legacy`).

These are the pre-refactor tests. They are what `io.legacy` was written against.
NOTE: Delete this file together with `jaxley/io/legacy.py`.
"""
...
# NOTE: `test_from_graph_vs_NEURON` was removed. Comp length, radius,
# area and volume are checked against NEURON for every backend in
# `test_swc.py::test_swc_morph_params_vs_neuron`, and the comp centers in
# `test_swc.py::test_comp_centers_agree_across_backends`.
...
def test_morph_edit_swc(ncomp: int):
    ...
    cell = jx.read_swc(fname, ncomp=ncomp, backend="legacy")
